...
GRHLEVEL_PROBLEM = 3
GRHLEVEL_OTHER = 4
GRFOOTER = 'Please close this issue once you have viewed your grade report, '
GRFOOTER += 'or let the grader know if you need further details about this grade report '
GRFOOTER += '(see instructions below).\n\n'
GRFOOTER += '### Disputes\n\n'
GRFOOTER += 'If you wish to discuss any matters regarding your score on this assignment, '
GRFOOTER += 'please write it as a comment on this issue and assign your grader. '
GRFOOTER += 'Any and all discussions pertaining to your homework score **must** be done via '
GRFOOTER += 'comments on this issue.  Do not use email to discuss grading matters as they won\'t '
GRFOOTER += 'be considered part of the regrade process. All discussions will be done privately '
GRFOOTER += 'using this issue.\n\n'
GRFOOTER += 'You have 7 days from the time this issue was posted to *raise* a dispute about any '
GRFOOTER += 'grading issues. Allow up to 48 hours for your grader to respond. If your grader '
GRFOOTER += 'cannot resolve the issue within 7 days, you must elevate the issue to one of the TAs '
GRFOOTER += '(Hannes, Gozde, or Yeji) by assigning them to this issue or attending their office '
GRFOOTER += 'hours. If after an additional 7 days you are still unhappy with the grading result, '
GRFOOTER += 'you must elevate the issue to a professor within 3 days. The professors will have the '
GRFOOTER += 'final input.\n\n'
GRFOOTER += 'This means you have a maximum of 7 + 7 + 3 days to resolve all grading disputes, '
GRFOOTER += 'after which point all grades are final.\n\n'
GRFOOTER += 'You are expected to follow the _Grading Disputes_ policies outlined on the '
GRFOOTER += '[assignments](http://bits.usc.edu/cs104/assignments) page.'
...
class Homework:
    ...
    def write_score_breakdown(self, report_file):
        ...
        for problem in self.problems:
            ...
            self.problem_subtotal.append(str(points))
        ...
        for deduct in self.other_deductions:
            ...
            deduction_text = '(-{pt} points) {comment}'.format(pt=deduct[0], comment=deduct[1])
        ...
        summary_text += ' **{}/{} points**.'.format(self.total_points, self.total_max)
...
class Problem:
    ...
    def _grade_valgrind_result(self):
        zerr = res.read_valgrind_result(self.valgrind_file)
        failed_count = self.test_count - zerr
        if failed_count < 0:
            log.log_error(
                'Found {} more valgrind output than test cases'.format(str(failed_count)),
                self.logging_level)
        elif failed_count > 0:
            valgrind_max = rb.get_valgrind_deduction_limit(self.rubric)
            deduction = rb.get_valgrind_deduction(self.rubric, failed_count)
            if deduction <= valgrind_max or valgrind_max < 0:
                self._add_deduction(
                    rb.get_valgrind_deduction(self.rubric, failed_count),
                    'Valgrind error')
            else:
                self._add_deduction(
                    valgrind_max,
