...
def run_gtest(executable_path, **kwargs):
    ...
    killed, _, _ = exe.run_executable(executable_path, **kwargs)
    return killed
...
#     kwargs:           See executable_tools.run_executable for more options.
# Return Values:
#     Returns a list of execution result.
#     EXE_ERROR if there's an error and the executable did not run.
#     EXE_SUCCESS if there's no timeout.
#     EXE_TIMEOUT if there's timeout.
####################################################################################################
def run_gtest_with_filter(executable_path, gtest_filters, **kwargs):
    ...
    for filtered_test in gtest_filters:
        ...
        killed, _, _ = exe.run_executable(exe.fix_executable_path(executable_path), **kwargs)
...
def run_gtest_in_file(source_file, target_file, output_result, **kwargs):
    ...
    run_result = run_gtest_with_filter(
        target_file,
        test_sorted,
        **kwargs,
    )
    ...
    return test_sorted, norun, timeout, crashed, failed, passed
...
#     See executable_tools.run_executable for all execution options.
# Return Values:
#     Three sets and two dictionaries.
#     1. set of tests that are not ran.
#     2. set of tests that timed out.
#     3. set of tests that crashed.
#     4. dictionary of tests that failed with their runtime.
#     5. dictionary of tests that succeeded with their runtime.
####################################################################################################
def run_gtest_in_dir(test_sorted,
    ...
    return norun, timeout, crashed, failed, passed
